...
use crate::config;
use crate::model::PersistedQueue;
use crate::Track;
...
impl PersistenceService {
    ...
    pub fn load_queue(&self) -> Result<PersistedQueue> {
        let path = self.config_dir.join("queue.json");

        let mut file = match fs::File::open(&path) {
            Ok(f) => f,
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
                return Ok(PersistedQueue {
                    tracks: Vec::new(),
                    current_track: None,
                });
            }
        ...
        let mut queue_state: PersistedQueue =
    ...
    pub fn save_queue(&self, state: &PersistedQueue) -> Result<()> {
        let path = self.config_dir.join("queue.json");
    ...
    pub fn load_download_cache(&self) -> HashMap<String, String> {
    ...
    pub fn save_download_cache(&self, cache: &HashMap<String, String>) -> Result<()> {
        let path = self.config_dir.join("download_cache.json");
    ...
    pub fn clear_playback_state(&self) {
        let path = self.config_dir.join("playback_state.json");
...
#[allow(dead_code)]
pub fn search_history<'a>(history: &'a [Track], query: &str) -> Vec<&'a Track> {
    if query.is_empty() {
